...
app.stop();
var loader = app.loader; // load spine data

loader.add('pixie', 'examples/assets/pixi-spine/pixie.json').add('bg', 'examples/assets/pixi-spine/iP4_BGtile.jpg').add('fg', 'examples/assets/pixi-spine/iP4_ground.png').load(onAssetsLoaded);
var objs = [];
var pixie = void 0;
app.stage.interactive = true; // 1 earth and 2 parallax layers
...
camera.addChild(groundLayer); // Those two layers can have 2d objects inside
...
function onAssetsLoaded(loaderInstance, res) {
  ...
  for (var _i = 0; _i < repeats; _i++) {
    // 3d sprite on floor
    var fg = new PIXI.projection.Sprite3d(res.fg.texture);
    groundLayer.addChild(fg);
    fg.anchor.set(0, 0.5); // use position or position3d here, its not important,
    // unless you need Z - then you need position3d

    fg.position.x = fg.texture.width * _i;
    objs.push(fg);
  }
...
app.ticker.add(function (delta) {
  pixie.position.x += 10 * delta; // camera looks on pixi!
