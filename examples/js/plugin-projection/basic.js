...
squareY.position.set(app.screen.width / 2, app.screen.height - 100); // create a new Sprite from an image path

var container = new PIXI.projection.Container2d();
container.position.set(app.screen.width / 2, app.screen.height / 2);
app.stage.addChild(container);
app.stage.addChild(squareX);
app.stage.addChild(squareY); // add sprite itself

var bunny = new PIXI.projection.Sprite2d(PIXI.Texture.from('examples/assets/flowerTop.png'));
bunny.anchor.set(0.5);
bunny.scale.set(0.7);
container.addChild(bunny); // illuminate the sprite from two points!
...
lightX.alpha = 0.2;
container.addChildAt(lightX, 1); // Listen for animate update
...
addInteraction(squareY); // move the bunny too!
...
function onDragEnd(event) {
  ...
  obj.dragging = 0;
  obj.dragData = null; // set the interaction data to null
}
