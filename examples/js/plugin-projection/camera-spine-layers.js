...
var loader = app.loader; // create a new loader
...
loader.add('pixie', 'examples/assets/pixi-spine/pixie.json'); // begin load

loader.load(onAssetsLoaded); // holder to store aliens

var alienFrames = ['eggHead.png', 'flowerTop.png', 'helmlok.png', 'skully.png']; // create an empty container
...
function spawnAlien(d) {
  ...
  if (d < 4) {
    var frameName = alienFrames[d]; // if you want to use 3d transform for object, either create Sprite3d/Container3d
...
function onAssetsLoaded() {
  var earth = new PIXI.projection.Sprite3d(loader.resources.back.texture); // because earth is Sprite3d, we can access its euler angles
...
app.ticker.add(function () {
  ...
  alienContainer.children.forEach(function (plane) {
    if (plane.alwaysFront) {
      ...
      plane.children[0].euler.x = -Math.PI / 6; // 2. rotate sprite to the camera
