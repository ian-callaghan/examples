...
document.body.appendChild(app.view); // META STUFF, groups exist without stage just fine
...
var dragGroup = new PIXI.display.Group(2, false); // Shadows are the lowest

var shadowGroup = new PIXI.display.Group(-1, false); // specify display list component

app.stage = new PIXI.display.Stage();
app.stage.group.enableSort = true; // sorry, group cant exist without layer yet :(
...
blurFilter.blur = 0.5; // create a texture from an image path

var textureGreen = PIXI.Texture.from('examples/assets/bunny_green.png');
var textureBlue = PIXI.Texture.from('examples/assets/bunny_blue.png'); // make obsolete containers. Why do we need them?
...
for (var i = 0; i < 15; i++) {
  ...
  bunny.anchor.set(0.5); // that thing is required
...
for (var _i = 9; _i >= 0; _i--) {
  var _bunny = new PIXI.Sprite(textureBlue);

  _bunny.width = 50;
  _bunny.height = 50;

  _bunny.position.set(400 + 20 * _i, 400 - 20 * _i);

  _bunny.anchor.set(0.5); // that thing is required


  _bunny.parentGroup = blueGroup;
  bunniesBlue.addChild(_bunny);
  subscribe(_bunny);
  addShadow(_bunny);
}
