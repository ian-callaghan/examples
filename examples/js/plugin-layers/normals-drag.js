...
var HEIGHT = H / resolution; // LAYERS plugin is here: https://github.com/pixijs/pixi-display/tree/layers
...
var stage = app.stage = new PIXI.display.Stage(); // bg is first, its not lighted

var bg = new PIXI.TilingSprite(PIXI.Texture.from('examples/assets/p2.jpeg'), WIDTH, HEIGHT);
bg.tint = 0x808080;
stage.addChild(bg); // put all layers for deferred rendering of normals
...
diffuseBlackSprite.tint = 0; // without the black sprite, lighted elements will be transparent to background. Try remove that line
...
var dragGroup = new PIXI.display.Group(0, true); // dragged objects has to processed after sorted, so we need a flag here too

dragGroup.sortPriority = 1;
stage.addChild(new PIXI.display.Layer(dragGroup)); // LIGHT and its movement
...
var lightLoader = new PIXI.Loader.shareds.Loader();
lightLoader.baseUrl = 'https://cdn.rawgit.com/pixijs/pixi-lights/b7fd7924fdf4e6a6b913ff29161402e7b36f0c0f/';
lightLoader.add('block_diffuse', 'test/block.png').add('block_normal', 'test/blockNormalMap.png').load(onAssetsLoaded);
...
function onAssetsLoaded(loader, res) {
  ...
  for (var _i = 1; _i < 8; _i += 2) {
    stage.addChild(createBlock(100 + _i * 50, 100 + _i * 30));
  }
}

function createBlock(x, y) {
  var container = new PIXI.Container(); // we need to sort them before children go to respective layers
...
function onDragEnd() {
  if (this.dragging) {
    ...
    this.scale.x /= 1.1;
    this.scale.y /= 1.1; // set the interaction data to null
