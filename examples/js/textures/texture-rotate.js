...
function init() {
  ...
  var gridH = app.screen.height / 5 | 0; // normal rotations and mirrors

  for (var i = 0; i < 16; i++) {
    // create a new Sprite using rotated texture
    var dude = new PIXI.Sprite(textures[i < 8 ? i * 2 : (i - 8) * 2 + 1]);
    dude.scale.x = 0.5;
    dude.scale.y = 0.5; // show it in grid

    dude.x = offsetX + gridW * (i % 4);
    dude.y = offsetY + gridH * (i / 4 | 0);
    app.stage.addChild(dude);
    var text = new PIXI.Text("rotate = ".concat(dude.texture.rotate), {
