...
app.stage.addChild(background); // speed up the process, because OVERLAY and HARD_LIGHT will use copyTex instead of readPixels

app.stage.filters = [new PIXI.filters.AlphaFilter()];
app.stage.filterArea = app.screen; // create an array to store a reference to the dudes

var dudeArray = [];
var totaldudes = 20;
var texture = PIXI.Texture.from('examples/assets/flowerTop.png');

for (var i = 0; i < totaldudes; i++) {
  // create a new Sprite that uses the image name that we just generated as its source
  var dude = new PIXI.Sprite(texture); // setting renderer plugin 'picture', from pixi-picture
  ...
  dude.blendMode = Math.random() > 0.5 ? PIXI.BLEND_MODES.OVERLAY : PIXI.BLEND_MODES.HARD_LIGHT; // create some extra properties that will control movement
